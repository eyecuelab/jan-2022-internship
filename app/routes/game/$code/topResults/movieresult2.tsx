...
export default function MovieResult2(props: {
  onRequestClose: MouseEventHandler<HTMLButtonElement> | undefined;
}) {
  const { movie2, movie2Details, movie2Cast, movie2WatchProviders } =
    useLoaderData();
  ...
  const genresArr: any[] = [];
  movie2Details.genres.forEach(function (entry: { name: string }) {
    if (entry.name) {
  ...
  const directors: any[] = [];
  movie2Cast.crew.forEach(function (entry: { job: string; name: string }) {
  ...
  const actorsArr: any[] = [];
  movie2Cast.cast.forEach(function (entry: { name: string }) {
    if (entry.name) {
  ...
  const subScore = movie2Details.vote_average;
  const totalVotes = movie2Details.vote_count;
  const votes = (totalVotes / 1000).toFixed(0);
  const score = `${subScore}  (${votes}K)`;

  const trailerArr: any[] = [];
  movie2Details.videos.results.forEach(function (entry: {
    key: string;
    name: string;
  }) {
  ...
  let revenue;
  if (movie2Details.revenue === 0 || !movie2Details.budget) {
    revenue = "N/A";
  } else {
    revenue = `$ ${Math.round(movie2Details.revenue / 1000000)} millions`;
  }

  let budget;
  if (movie2Details.budget === 0 || !movie2Details.budget) {
    budget = "N/A";
  } else {
    budget = `$ ${Math.round(movie2Details.budget / 1000000)} millions`;
  }
  ...
  return (
    <>
      ...
      <header className="modal-header">
        <button className="modal-btn-number ">2</button>
        <ul style={{ paddingLeft: 12 }}>
          <li>
            <div className="modal-movie-title" title={movie2.results[0].title}>
        ...
        <div className="bio-page-wrapper">
          ...
          <div className="row-bio">
            <div className="col-bio">
              <p style={{ color: "#fff" }}>{movie2.results[0].overview}</p>
            </div>
          </div>
        ...
        <ul style={{ color: "#fff", marginTop: "1em" }}>
          <li>
            ...
            <div className="modal-row">
              {movie2WatchProviders.results?.US?.ads?.map(
              ...
              {movie2WatchProviders.results?.US?.flatrate?.map(
                (entry: { logo_path: Key | null | undefined }) => (
                  <div key={entry.logo_path} className="modal-block">
                    <img
                      src={LOGO_URL + entry.logo_path}
          ...
          <li>
            BUY
            <div className="modal-row">
              {movie2WatchProviders.results?.US?.buy?.map(
                (entry: { logo_path: Key | null | undefined }) => (
                  <div key={entry.logo_path} className="modal-block">
                    <img
                      src={LOGO_URL + entry.logo_path}
          ...
          <li>
            RENT
            <div className="modal-row">
              {movie2WatchProviders.results?.US?.rent?.map(
                (entry: { logo_path: Key | null | undefined }) => (
                  <div key={entry.logo_path} className="modal-block">
                    <img
                      src={LOGO_URL + entry.logo_path}
