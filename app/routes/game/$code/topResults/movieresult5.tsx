...
export const loader: LoaderFunction = async ({ request, params }) => {
  ...
  const movie5Details = await callApiGetDetails(String(movie5.results[0].id));
  const movie5Cast = await callApiGetCredits(String(movie5.results[0].id));

  return { topFive, slug, movie5, movie5Details, movie5Cast };
};

//Modal.setAppElement("#root");

export default function MovieResult5() {
  const { slug, movie5, movie5Details, movie5Cast } = useLoaderData();
  ...
  console.log(movie5);
  console.log(movie5Details);
  console.log(movie5Cast);
